# Robert Hall Programming assignment #7 pg. 141
# Takes in user_input and validates based on a set of rules.
# Contains statements in place to check for proper data entry.

import re
user_input = input("Enter a credit card number: ")
if bool(re.match('^[0-9]+$',user_input)):
    credit_card_number = [int(x) for x in str(user_input)]
    replacements = 0;
    odd_numbered = 0;
    index = 0;
